...
class TransformerDecoderLayer(nn.Module):
    def __init__(self,
        ...
        self.num = len(args.train_classes) * 2
        # Attention Layer
    ...
    def forward(self, vis, txt):
        ...
        vis2 = \
        self.multihead_attn(query=(vis2.permute(1, 0, 2) + self.positional_embedding.to(vis.dtype)).permute(1, 0, 2),
...
class PerFeatureFusion(nn.Module):
    def __init__(self, feature_dim, noise_std=1.0):
        super().__init__()
        self.D = feature_dim
        self.noise_std = noise_std
        # 每个维度都有独立 Linear(2 → 1)：参数不共享
        self.weight = nn.Parameter(torch.randn(self.D, 1, 2))  # (D, 1, 2)
        self.bias = nn.Parameter(torch.randn(self.D))          # (D,)

    def forward(self, vis, feature_back):
        """
        vis: (N, D)
        feature_back: (N, D)
        return: 
        ...
        noise = torch.randn_like(vis) * self.noise_std  # (N, D)

        pairwise_input = torch.stack([vis, noise], dim=-1)  # (N, D, 2)

        input_exp = pairwise_input.unsqueeze(-2)               # (N, D, 1, 2)
        weight_exp = self.weight.transpose(1, 2).unsqueeze(0)  # (1, D, 2, 1)
        fused = torch.matmul(input_exp, weight_exp).squeeze(-1).squeeze(-1)  # (N, D)

        output = fused + self.bias + feature_back  # (N, D)
        return output
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.fusion_layer = PerFeatureFusion(512)
    ...
    def forward(self, image, label = None):
        ...
        vis = self.ln_post(vis[:, 0, :])  # f_tar
        feature_back = image_features - vis  # f_ass

        logit_scale = self.logit_scale.exp()
        if label is None:
            logits_fake = None
        else:
            feature_fake = self.fusion_layer(vis, feature_back)
            feature_fake = feature_fake / feature_fake.norm(dim=-1, keepdim=True)
            logits_fake = logit_scale * feature_fake @ text_features.t()
        ...
        # logits_bg = logit_scale * feature_back @ text_features.t()


        return logits_for, logits_back, logits_com, logits_fake, simi_fb
