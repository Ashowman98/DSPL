...
import numpy as np
from config import cifar_10_root, cifar_100_root, cifar_oods_root
...
class CifarOOD(ImageFolder):

    def __init__(self, root, transform):

        super(CifarOOD, self).__init__(root, transform)
...
def get_cifar_10_oods_datasets(train_transform, test_transform, train_classes=range(4),
                       open_set_classes=range(4, 10), balance_open_set_eval=False, split_train_val=True, seed=0):
    ...
    # Get testset for unknown classes
    test_dataset_unknown1 = CifarOOD(root=os.path.join(cifar_oods_root, 'imagenet_crop'), transform=test_transform)
    # test_dataset_unknown1 = subsample_classes(test_dataset_unknown1, include_classes=open_set_classes)

    test_dataset_unknown2 = CifarOOD(root=os.path.join(cifar_oods_root, 'imagenet_resize'), transform=test_transform)
    # test_dataset_unknown2 = subsample_classes(test_dataset_unknown2, include_classes=open_set_classes)

    test_dataset_unknown3 = CifarOOD(root=os.path.join(cifar_oods_root, 'lsun_crop'), transform=test_transform)
    # test_dataset_unknown3 = subsample_classes(test_dataset_unknown3, include_classes=open_set_classes)

    test_dataset_unknown4 = CifarOOD(root=os.path.join(cifar_oods_root, 'lsun_resize'), transform=test_transform)
    # test_dataset_unknown4 = subsample_classes(test_dataset_unknown4, include_classes=open_set_classes)
    ...
    all_datasets = {
        ...
        'val': val_dataset,
        'test_known': test_dataset_known,
        'test_unknown1': test_dataset_unknown1,
        'test_unknown2': test_dataset_unknown2,
        'test_unknown3': test_dataset_unknown3,
        'test_unknown4': test_dataset_unknown4,
    }
