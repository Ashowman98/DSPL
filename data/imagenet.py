...
def subsample_dataset(dataset, idxs):
    idxs = np.array(idxs)  # 转换为 numpy array，加速索引

    dataset.imgs = [dataset.imgs[i] for i in idxs]
    dataset.samples = [dataset.samples[i] for i in idxs]
    dataset.targets = np.array(dataset.targets)[idxs].tolist()
    dataset.uq_idxs = dataset.uq_idxs[idxs]  # uq_idxs 是 numpy，直接索引更快
...
def get_image_net_datasets(train_transform, test_transform, seed=0):
    ...
    train_classes = [i for i in range(100)]
    ...
    test_dataset_known = ImageNetBase(root=os.path.join(imagenet_root, 'new_val'), transform=test_transform)
    ...
    test_dataset_unknown = ImageNetBase(root=os.path.join(imagenet_root, 'new_val'), transform=test_transform)
    # Select which classes are open set
    open_set_classes = [i for i in range(100, 1000)]
